...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const geminiResponse = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/${MODEL}:generateContent`,
      {
        ...
        body: JSON.stringify({
          systemInstruction: {
            parts: [{
              text: 'You are BUNVIX AI, a helpful, accurate, concise and friendly AI assistant. Be clear, useful and honest about uncertainty. Never claim to have performed actions you did not perform.',
            }],
          },
    ...
    if (!geminiResponse.ok) {
      console.error('Gemini API error:', geminiResponse.status, result);
      const apiMessage = typeof result?.error?.message === 'string'
    ...
    if (!answer) {
      ...
      return json({ error: 'Gemini returned no text response.' }, 502);
    }
    ...
    console.error('BUNVIX chat error:', error);
